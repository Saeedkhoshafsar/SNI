...
## ۱۴. اصلاحیه (Phase 4.4) — fail-fastِ دست‌دادن + رفعِ نوسانِ سرعت و سیلِ لاگ

لاگِ ۲۲:۰۲–۲۲:۰۷ نشان داد churn کاملاً متوقف شده و تونل کار می‌کند، ولی دو نشانه ماند:
(۱) `TimeoutError`ِ پراکنده و (۲) **نوسانِ شدیدِ سرعت** (۱.۶MB → ۳.۴KB) به‌صورتِ متناوب.

**ریشه‌یابی (با خواندنِ `fake_tcp.py` + `main.py`):**
- استراتژیِ `wrong_seq` پس از تزریقِ ClientHelloِ جعلی منتظرِ یک **duplicate-ACK** از
  CDN می‌ماند (`on_inbound_packet` → `fake_data_ack_recv`). این ACK معمولاً در کسری از
  ثانیه می‌آید، ولی گاهی (وابسته به شبکه/CDN) برنمی‌گردد.
- مشکل: `_handle` تا **۵ ثانیه** روی این ACK بلاک می‌شد. وقتی برنمی‌گشت، آن streamِ xray
  ۵ ثانیهٔ کامل یخ می‌زد و دانلودِ کاربر می‌افتاد، بعد xray retry می‌کرد → دقیقاً همان
  الگوی «سرعت یهو می‌افتد و دوباره بالا می‌رود».

**رفع (بدونِ دست‌زدن به مسیرِ داده‌ی زنده، پینگ، یا کانفیگ‌های معمولی):**
1. **`FAKE_HANDSHAKE_TIMEOUT = 1.5`** (به‌جای ۵ ثانیه): یک duplicate-ACKِ واقعی در یک RTT
   (ده‌ها میلی‌ثانیه) می‌آید؛ اگر نیامد یعنی اتصال مرده است و بلاک‌ماندنِ ۵ ثانیه‌ای فقط
   ضرر است. با fail-fast، muxِ xray فوراً streamِ گیرکرده را رها می‌کند و روی یک اتصالِ
   تازه که دست‌دادنش موفق می‌شود retry می‌کند — همین نوسانِ سرعت را صاف می‌کند.
2. **`_log_fake_fail` با rate-limit:** خطاهای تکراریِ «دست‌دادن جعلی شکست/تایم‌اوت» در یک
   پنجره‌ی ۴ ثانیه‌ای در یک خطِ واحد (با شمارش) جمع می‌شوند تا لاگ خوانا بماند، با راهنماییِ
   عملی (امتحانِ استراتژی/IP/SNIِ دیگر، اطمینان از Admin/WinDivert).

**نکته:** علتِ ریشه‌ایِ نیامدنِ duplicate-ACK، رفتارِ شبکه/CDN است (نه باگِ کد)؛ این اصلاح
اثرِ آن را به حداقل می‌رساند (fail-fast + لاگِ تمیز) تا کاربر بتواند استراتژی/مسیرِ بهتری
انتخاب کند.

**فایل تغییر یافته:** `main.py` (ثابتِ timeout + helperِ rate-limited + استفاده در هر دو
مسیرِ expects_ack و fire-and-forget).

**تست:** ۶۷ تست `test_pool.py` + ۱۷ تست `test_proxy_startup.py` + ۱۹۱ تست مرتبط، همه سبز.
